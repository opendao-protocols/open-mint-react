...
import { LS_KEY_CONNECTED_CONNECTOR } from "../../lib/const";
import toast from "../../components/Common/Toast";
declare var window: any;

const getConnectedConnector = (): Connector | undefined => {
  const lsConnectedConnector =
    !!process.browser &&
    window.localStorage.getItem(LS_KEY_CONNECTED_CONNECTOR);
...
const useAuth = () => {
  ...
  const login = useCallback(
    async (connectorID: Connector) => {
      ...
      if (!connector) {
        ...
        toast.error({
          title:
            "An internal error occurred: unsupported wallet. Please try again later",
        });
        return;
      }
      ...
      try {
        ...
        if (
          ...
          errorMessage =
            "An internal error occurred: no provider found. Please try again later";
        } else {
  ...
  const logOut = useCallback(() => {
    ...
    if (window.localStorage.getItem("walletconnect")) {
      ...
      connectorsByName[Connector.WalletConnect].walletConnectProvider =
        undefined;
    }
